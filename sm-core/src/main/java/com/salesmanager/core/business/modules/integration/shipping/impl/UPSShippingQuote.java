...
public class UPSShippingQuote implements ShippingQuoteModule {
	...
	public List<ShippingOption> getShippingQuotes(ShippingQuote shippingQuote, List<PackageDetails> packages,
		...
		if (!(country.getIsoCode().equals("US") || country.getIsoCode().equals("CA"))) {
			return null;
			// throw new IntegrationException("UPS Not configured for shipping in country "
			// + country.getIsoCode());
		}
		...
		try {
			 ...
			 * <Shipment>
			 * 
			 * <Shipper> <Address> <City></City> <StateProvinceCode>QC</StateProvinceCode>
			 * <CountryCode>CA</CountryCode> <PostalCode></PostalCode> </Address> </Shipper>
			 * 
			 * <ShipTo> <Address> <City>Redwood Shores</City>
			 * <StateProvinceCode>CA</StateProvinceCode> <CountryCode>US</CountryCode>
			 * <PostalCode></PostalCode> <ResidentialAddressIndicator/> </Address> </ShipTo>
			 * 
			 * <Package> <PackagingType> <Code>21</Code> </PackagingType> <PackageWeight>
			 * <UnitOfMeasurement> <Code>LBS</Code> </UnitOfMeasurement>
			 * <Weight>1.1</Weight> </PackageWeight> <PackageServiceOptions> <InsuredValue>
			 * <CurrencyCode>CAD</CurrencyCode> <MonetaryValue>100</MonetaryValue>
			 * </InsuredValue> </PackageServiceOptions> </Package>
			 * 
			 * 
			 * </Shipment>
			 * 
			 * <CustomerClassification> <Code>03</Code> </CustomerClassification>
			 * </RatingServiceSelectionRequest>
			 **/

			/**
			 * Map countriesMap = (Map) RefCache.getAllcountriesmap(LanguageUtil
			 * .getLanguageNumberCode(locale.getLanguage())); Map zonesMap = (Map)
			 * RefCache.getAllZonesmap(LanguageUtil
			 ...
			 * 
			 * int sZone = -1; try { sZone = Integer.parseInt(store.getZone()); } catch
			 * (Exception e) { // TODO: handle exception }
			 * 
			 * Zone storeZone = (Zone) zonesMap.get(sZone); Zone customerZone = (Zone)
			 * zonesMap.get(customer .getCustomerZoneId());
			 **/
			...
			// xmldatabuffer.append("<Service><Code>11</Code></Service>");//TODO service
			// codes (next day ...)
			...
			for (PackageDetails packageDetail : packages) {
				...
				xmldatabuffer
						.append(new BigDecimal(packageDetail.getShippingWidth()).setScale(2, BigDecimal.ROUND_HALF_UP));
				xmldatabuffer.append("</Width>");
			...
			/*
			 * if (result != 200) { LOGGER.error("Communication Error with ups quote " +
			 * result + " " + protocol + "://" + host + ":" + port + url); throw new
			 * Exception("UPS quote communication error " + result); }
			 */
			...
			/*
			 * String carrier = getShippingMethodDescription(locale); // cost is in CAD,
			 * need to do conversion
			 * 
			 * 
			 * boolean requiresCurrencyConversion = false; String storeCurrency =
			 * store.getCurrency(); if(!storeCurrency.equals(Constants.CURRENCY_CODE_CAD)) {
			 * requiresCurrencyConversion = true; }
			 * 
			 * 
			 * LabelUtil labelUtil = LabelUtil.getInstance(); Map serviceMap =
			 * com.salesmanager.core.util.ShippingUtil .buildServiceMap("upsxml", locale);
			 * 
			 *//** Details on whit RT quote information to display **//*
																		 * MerchantConfiguration rtdetails = config
																		 * .getMerchantConfiguration(ShippingConstants.
																		 * MODULE_SHIPPING_DISPLAY_REALTIME_QUOTES); int
																		 ...
																		 * getConfigurationValue1())) {// display // or
																		 * // not // quotes try {
																		 * displayQuoteDeliveryTime =
																		 * Integer.parseInt(rtdetails
			 ...
			 * 
			 * Iterator i = options.iterator(); while (i.hasNext()) { ShippingOption option
			 * = (ShippingOption) i.next(); // option.setCurrency(store.getCurrency());
			 * StringBuffer description = new StringBuffer();
			 * 
			 * String code = option.getOptionCode(); option.setOptionCode(code); // get
			 * description String label = (String) serviceMap.get(code); if (label == null)
			 ...
			 * ShippingConstants.DISPLAY_RT_QUOTE_TIME) { if (!StringUtils.isBlank(option
			 * .getEstimatedNumberOfDays())) { description.append(" (").append(
			 * option.getEstimatedNumberOfDays()).append( " ").append(
			 * labelUtil.getText(locale, "label.generic.days.lowercase")) .append(")"); } }
			 * option.setDescription(description.toString());
			 * 
			 * // get currency if (!option.getCurrency().equals(store.getCurrency())) {
			 * option.setOptionPrice(CurrencyUtil.convertToCurrency(
			 * option.getOptionPrice(), option.getCurrency(), store.getCurrency())); }
			 * 
			 * if (!selectedintlservices.containsKey(option .getOptionCode())) { if
			 * (returnColl == null) { returnColl = new ArrayList(); }
			 * returnColl.add(option); // options.remove(option); }
			 * 
			 * }
			 * 
			 * if (options.size() == 0) { LogMerchantUtil .log( store.getMerchantId(),
			 * " none of the service code returned by UPS [" + selectedintlservices
			 * .keySet() .toArray( new String[selectedintlservices .size()]) +
	...
	public void validateModuleConfiguration(IntegrationConfiguration integrationConfiguration, MerchantStore store)
		 ...
		 * 
		 * if(services!=null && services.size()>3) { if(errorFields==null) { errorFields
		 * = new ArrayList<String>(); } errorFields.add("services"); }
		 */
